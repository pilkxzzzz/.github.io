...
async function loginUser(username, password) {
    try {
        ...
        if (user && user.password === password) {
            console.log('Пароль співпада, вхід успішний');
            return user;
        } else {
...
async function loadProfile() {
    ...
    try {
        ...
        console.log('Профіль успішно завантажено');
    } catch (error) {
...
async function sendMessageToUser(username) {
    if (!isAdmin(currentUser)) {
        showMessage('У вас немає прав а��міністратора');
        return;
    }
    ...
    try {
        ...
        showMessage('Помилка відпр��вки повідомлення');
    }
}
...
function setupButtonHandlers() {
    ...
    // Кнопка для показу форми ручного введення
...
async function disconnectTelegram() {
    ...
    try {
        ...
        showMessage('Пом��лка відключення Telegram');
    }
}
...
async function registerUser(username, password) {
    try {
        ...
        if (existingUsers.length > 0) {
            showMessage('Користувач з таким нікнейм вже існує');
            return null;
        }

        // Створюємо нового користувача з ID
        const newUser = {
...
async function toggleLike(username) {
    ...
    try {
        ...
        if (isLiked) {
            showMessage('Ви вже вподобали цей профіль');
            return;
        }
...
async function acceptLike(fromUsername) {
    ...
    try {
        // Отримуємо дані користувача, який поставив лайк
